    ...
    **Attributes**
        ...
        **Raises**
        ...
        if self.kind == "function":
            return "{template}{return_type} {name}({parameters})".format(
                template="template <{}> ".format(", ".join(self.template)) if self.template is not None else "",
            ...
            for param_t, decl_n, def_n in self.template_params:
                ...
                if refid:
                    ...
                    if refid in nodeByRefid:
                        link = f"{nodeByRefid[refid].link_name}_"
                    else:
    ...
    def inHierarchy(self, hierarchyType):
        if hierarchyType == "page":
            ...
            raise RuntimeError(f"'{hierarchyType}' is not a valid hierarchy type")

    def hierarchySortedDirectDescendants(self, hierarchyType):
        if hierarchyType == "page":
            if self.kind != "page":
                raise RuntimeError(
                    f"Page hierarchies do not apply to '{self.kind}' nodes"
                )
    ...
    **Attributes**
    ...
    def discoverAllNodes(self):
        ...
        try:
            ...
            raise RuntimeError(f"Could not read the contents of [{doxygen_index_xml}].")

        try:
            index_soup = BeautifulSoup(index_contents, "lxml-xml")
        except:
            raise RuntimeError(f"Could not parse the contents of [{doxygen_index_xml}] as an xml.")

        doxygen_root = index_soup.doxygenindex
        if not doxygen_root:
            raise RuntimeError(
                f"Did not find root XML node named 'doxygenindex' parsing [{doxygen_index_xml}]."
            )
        ...
        for page in self.pages:
            ...
            if node_xml_contents:
                try:
                    ...
                    utils.fancyError(f"Unable to parse file xml [{page.name}]:")
                ...
                try:
                    ...
                    utils.verbose_log(
                        f"*** [{page.name}] had [{len(inner_pages)}] innerpages found",
                    ...
                    utils.fancyError(
                        f"Could not process Doxygen xml for file [{f.name}]"
                    )
        ...
        for f in self.files:
            ...
            if node_xml_contents:
                try:
                    ...
                    utils.fancyError(f"Unable to parse file xml [{f.name}]:")
                ...
                try:
                    ...
                    # << verboseBuild
                    utils.verbose_log(
                        f"*** [{f.name}] had [{len(inner_classes)}] innerclasses found",
                        utils.AnsiColors.BOLD_MAGENTA
                    )
                    ...
                    # << verboseBuild
                    utils.verbose_log(
                        f"*** [{f.name}] had [{len(memberdefs)}] memberdef",
                        utils.AnsiColors.BOLD_MAGENTA
                    )
        ...
        for refid in missing_file_def:
            ...
            if len(candidates) == 1:
                ...
                utils.verbose_log(utils.info(
                    "Manually setting file definition of {} {} to [{}]".format(
            ...
            # More than one found, don't know what to do...
                ...
                err_msg.write(textwrap.dedent('''
                    While attempting to discover the file that Doxygen refid `{}` was
                ...
                for c in candidates:
                    err_msg.write(f"  - path=[{c.location}], refid={c.refid}\n")
                err_msg.write("\n")
        ...
        for node in self.class_like:
            ...
            if node_xml_contents:
                try:
                    ...
                    utils.fancyError("Could not process [{}]".format(
                        os.path.join(configs._doxygen_xml_output_directory, f"{node.refid}")
                    ))
                ...
                try:
                    ...
                    utils.fancyError(f"Error processing Doxygen XML for [{node.name}]", "txt")
        ...
        for u in self.unions:
            ...
            if len(parts) >= 2:
                ...
                if reparented:
                    ...
                    # << verboseBuild
                    utils.verbose_log(
                        f"The union {u.name} has '::' in its name, but no parent was found!",
                        utils.AnsiColors.BOLD_RED
                    )
    ...
    def renameToNamespaceScopes(self):
        ...
        for n in self.namespaces:
            namespace_name = f"{n.name}::"
            for child in n.children:
                if namespace_name not in child.name:
                    child.name = f"{namespace_name}{child.name}"
    ...
    def fileRefDiscovery(self):
        ...
        if not os.path.isdir(configs._doxygen_xml_output_directory):
            utils.fancyError("The doxygen xml output directory [{}] is not valid!".format(
                configs._doxygen_xml_output_directory
            ))
        ...
        for f in self.files:
            ...
            try:
                doxy_xml_path = os.path.join(configs._doxygen_xml_output_directory, f"{f.refid}.xml")
                ...
                utils.fancyError(
                    f"Unable to process doxygen xml for file [{f.name}].\n"
                )
        ...
        for f in self.files:
            for child in f.children:
                if child.def_in_file is None:
                    ...
                    # << verboseBuild
                    utils.verbose_log(
                        "Conflicting file definition for [{}]: both [{}] and [{}] found.".format(
        ...
        for f in self.files:
            if not f.location:
                sys.stderr.write(utils.critical(
                    "Cannot reparent file [{}] because it's location was not discovered.\n".format(
                        f.name
                    )
                ))
                continue
                ...
                utils.verbose_log(
                    "### File [{}] with location [{}] was identified as being at the top level".format(
                        f.name, f.location
                    ),
                    utils.AnsiColors.BOLD_YELLOW
                )
            ...
            if not found:
                sys.stderr.write(utils.critical(
                    "Could not find directory parent of file [{}] with location [{}].\n".format(
                        f.name, f.location
                    )
                ))
            ...
            if func.parent:
                ...
                if func.parent.kind == "namespace":
                    ...
                    raise RuntimeError(textwrap.dedent('''
                        Function [{}] with refid=[{}] had a parent of kind '{}':
                        Parent name=[{}], refid=[{}].
    ...
    def generateFullAPI(self):
        ...
        try:
            ...
            utils.fancyError(
                f"Cannot create the directory {self.root_directory} {e}"
            )
        ...
        try:
            with codecs.open(self.full_root_file_path, "w", "utf-8") as generated_index:
                ...
                if configs.pageLevelConfigMeta:
                    generated_index.write(f"{configs.pageLevelConfigMeta}\n\n")
                ...
                if configs.afterTitleDescription:
                    generated_index.write(f"\n{configs.afterTitleDescription}\n\n")
        except:
            utils.fancyError(
                f"Unable to create the root api file / header: {self.full_root_file_path}"
            )
    ...
    def initializeNodeFilenameAndLink(self, node):
        ...
        if node.kind in SPECIAL_CASES:
            node.link_name = f"{node.kind}_{unique_id}"
            node.file_name = f"{node.link_name}.rst"
            ...
            node.link_name = f"exhale_{node.kind}_{unique_id}"
            if unique_id.startswith(node.kind):
                node.file_name = f"{unique_id}.rst"
            else:
                node.file_name = f"{node.kind}_{unique_id}.rst"
        ...
        if node.kind == "file":
            node.program_link_name = f"program_listing_{node.link_name}"
            node.program_file = f"{node.program_link_name}.rst"
            ...
            if len(node.program_file) >= configs.MAXIMUM_FILENAME_LENGTH:
                ...
                node.file_name = f"{node.kind}_{sha1}.rst"
                node.program_file = f"program_listing_{node.file_name}"
            ...
            for parameters_str in parameter_warning_map:
                ...
                if len(warn_functions) > 1:
                    sys.stderr.write(utils.critical(
                        ...
                        "".join([f"\n- {wf.full_signature()}" for wf in warn_functions]) + \
                        textwrap.dedent('''
    ...
    def generateSingleNodeRST(self, node):
        ...
        try:
            with codecs.open(node.file_name, "w", "utf-8") as gen_file:
                ...
                link_declaration = f".. _{node.link_name}:"

                # acquire the file this was originally defined in
                if node.def_in_file:
                    defined_in = f"- Defined in :ref:`{node.def_in_file.link_name}`"
                else:
                    defined_in = ".. did not find file this was defined in"
                    sys.stderr.write(utils.critical(
                        "Did not locate file that defined {} [{}]; no link generated.\n".format(node.kind,
                                                                                                  node.name)
                    ))

                # Add the metadata if they requested it
                if configs.pageLevelConfigMeta:
                    gen_file.write(f"{configs.pageLevelConfigMeta}\n\n")
                ...
                if node.kind == "class" or node.kind == "struct":
                    ...
                    if nested_children:
                        ...
                        for nc in nested_children:
                            nested_child_stream.write(f"- :ref:`{nc.link_name}`\n")
                        ...
                        nested_defs = f"{nested_defs}{nested_child_string}\n"
                ...
                if nested_type_of or nested_defs:
                    ...
                    if nested_type_of:
                        gen_file.write(f"{nested_type_of}\n\n")
                    if nested_defs:
                ...
                ########################################################################
                heading = f"{utils.qualifyKind(node.kind)} Documentation"
                gen_file.write(textwrap.dedent('''
                    {heading}
                    {heading_mark}
            ...
            utils.fancyError(
                f"Critical error while generating the file for [{node.file_name}]."
            )
    ...
    def generateSingleNamespace(self, nspace):
        ...
        try:
            with codecs.open(nspace.file_name, "w", "utf-8") as gen_file:
                ...
                nspace.title = f"{utils.qualifyKind(nspace.kind)} {nspace.name}"
            ...
            utils.fancyError(
                f"Critical error while generating the file for [{nspace.file_name}]"
            )
    ...
    def generateFileNodeDocuments(self):
        ...
        for f in self.files:
            ...
            if len(f.program_listing) > 0:
                ...
                full_program_listing = f'.. code-block:: {lexer}\n\n'
                ...
                for pgf_line in f.program_listing:
                    ...
                    full_program_listing = f"{full_program_listing}   {revive_amp}"
                ...
                try:
                    with codecs.open(f.program_file, "w", "utf-8") as gen_file:
                        ...
                        # generate a link label for every generated file
                        link_declaration = f".. _{f.program_link_name}:"
                        # every generated file must have a header for sphinx to be happy
                        prog_title = f"Program Listing for {utils.qualifyKind(f.kind)} {f.name}"
                        gen_file.write(textwrap.dedent('''
                            {link}
                    ...
                    utils.fancyError(
                        f"Critical error while generating the file for [{f.file_name}]"
                    )
            else:
        ...
        for f in self.files:
            if len(f.location) > 0:
                heading = f"Definition (``{f.location}``)"
            ...
            if include_program_listing and file_definition != "":
                ...
                file_definition = f"{file_definition}{prog_file_definition}"
            ...
            try:
                with codecs.open(f.file_name, "w", "utf-8") as gen_file:
                    ...
                    # generate a link label for every generated file
                    link_declaration = f".. _{f.link_name}:"
                    # every generated file must have a header for sphinx to be happy
                    f.title = f"{utils.qualifyKind(f.kind)} {f.name}"
                    gen_file.write(textwrap.dedent('''
                        {link}
            ...
            if configs.generateBreatheFileDirectives:
                try:
                    ...
                    utils.fancyError(
                        f"Critical error while generating the breathe directive for [{f.file_name}]"
                    )
    ...
    def generateDirectoryNodeRST(self, node):
        ...
        if len(child_dirs) > 0:
            ...
            for child_dir in sorted(child_dirs):
                child_dirs_string = f"{child_dirs_string}- :ref:`{child_dir.link_name}`\n"
        else:
            child_dirs_string = ""
        ...
        if len(child_files) > 0:
            ...
            for child_file in sorted(child_files):
                child_files_string = f"{child_files_string}- :ref:`{child_file.link_name}`\n"
        else:
            child_files_string = ""
    ...
    def generateAPIRootBody(self):
        ...
        try:
            ...
            with codecs.open(self.full_root_file_path, "a", "utf-8") as generated_index:
                ...
                if configs.afterHierarchyDescription:
                    generated_index.write(
                        f"\n{configs.afterHierarchyDescription}\n\n"
                    )
            ...
            utils.fancyError(
                f"Unable to create the root api body: [{self.full_root_file_path}]"
            )
    ...
    def writeOutHierarchy(self, hierarchy_config, data):
        ...
        if configs.createTreeView:
            ...
            if data:
                ...
                indented_data = re.sub(r'(.+)', fr'{indent}\1', data)
        ...
        try:
            ...
            utils.fancyError(f"Error writing the {h_type} hierarchy.")
        ...
        class UnabridgedDict(MutableMapping):
            ...
            def __getitem__(self, key):
                ...
                if k not in self.items:
                    sys.stderr.write(utils.critical(
                        f"Unabridged API: unexpected kind '{key}' (IGNORED)\n"
                    ))
                    self.items[k] = []
                return self.items[k]
