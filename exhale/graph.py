            ...
            for param_t, decl_n, def_n in self.template_params:
                ...
                if refid:
                    ...
                    if refid in nodeByRefid:
                        link = "{}_".format(nodeByRefid[refid].link_name)
                    else:
        ...
        for prot, refid, string in lst:
            ...
            if refid and refid in nodeByRefid:
                ...
                if not please_close:
                    ...
                    bod_stream.write(" ")
                bod_stream.write("{}``".format(string))
            bod_stream.write("\n")
    ...
    **Attributes**
        ...
        for page in self.pages:
            ...
            if node_xml_contents:
                try:
                    ...
                    utils.fancyError("Unable to parse file xml [{}]:".format(page.name))
                ...
                try:
                    ...
                    utils.fancyError(
                        "Could not process Doxygen xml for file [{}]".format(f.name)
                    )
        ...
        for f in self.files:
            ...
            if node_xml_contents:
                try:
                    ...
                    utils.fancyError("Unable to parse file xml [{}]:".format(f.name))
                ...
                try:
                    ...
                    # << verboseBuild
                    utils.verbose_log(
                        "*** [{}] had [{}] innerclasses found".format(f.name, len(inner_classes)),
                        utils.AnsiColors.BOLD_MAGENTA
                    )
            ...
            if node_xml_contents:
                try:
                    ...
                    utils.fancyError("Could not process [{}]".format(
                        os.path.join(configs._doxygen_xml_output_directory, "{}".format(node.refid))
                    ))
                ...
                try:
                    ...
                    utils.fancyError("Error processing Doxygen XML for [{}]".format(node.name), "txt")
        ...
        for u in self.unions:
            ...
            if len(parts) >= 2:
                ...
                if reparented:
                    ...
                    # << verboseBuild
                    utils.verbose_log(
                        "The union {} has '::' in its name, but no parent was found!".format(u.name),
                        utils.AnsiColors.BOLD_RED
                    )
    ...
    def fileRefDiscovery(self):
        ...
        for f in self.files:
            ...
            try:
                doxy_xml_path = os.path.join(configs._doxygen_xml_output_directory, "{}.xml".format(f.refid))
                ...
                utils.fancyError(
                    "Unable to process doxygen xml for file [{}].\n".format(f.name)
                )
    ...
    def generateFullAPI(self):
        ...
        try:
            ...
            utils.fancyError(
                "Cannot create the directory {} {}".format(self.root_directory, e)
            )
    ...
    def generateAPIRootHeader(self):
        ...
        try:
            ...
            utils.fancyError(
                "Unable to create the root api file / header: {}".format(self.full_root_file_path)
            )
            ...
            for parameters_str in parameter_warning_map:
                ...
                if len(warn_functions) > 1:
                    sys.stderr.write(utils.critical(
                        ...
                        "".join(["\n- {}".format(wf.full_signature()) for wf in warn_functions]) + \
                        textwrap.dedent('''
    ...
    def generateSingleNodeRST(self, node):
        ...
        try:
            ...
            utils.fancyError(
                "Critical error while generating the file for [{}].".format(node.file_name)
            )
    ...
    def generateSingleNamespace(self, nspace):
        ...
        try:
            ...
            utils.fancyError(
                "Critical error while generating the file for [{}]".format(nspace.file_name)
            )
    ...
    def generateFileNodeDocuments(self):
        ...
        for f in self.files:
            ...
            if len(f.program_listing) > 0:
                ...
                full_program_listing = '.. code-block:: {}\n\n'.format(lexer)
                ...
                try:
                    ...
                    utils.fancyError(
                        "Critical error while generating the file for [{}]".format(f.file_name)
                    )
            else:
    ...
    def generateAPIRootBody(self):
        ...
        try:
            ...
            utils.fancyError(
                "Unable to create the root api body: [{}]".format(self.full_root_file_path)
            )
